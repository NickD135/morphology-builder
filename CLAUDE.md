...
- [x] **Teacher full shop access** (`96a6c9d`, detection fixed `f80051f`): a teacher previewing the scientist shop gets `WL.isTeacher` → every item across all categories, incl. badge-gated cosmetics, renders owned+unlocked and equips free (no cost/badge check); Surprise-me draws the full catalogue. Student experience unchanged; no badge data faked (student records not polluted). Custom teacher-made items keep their own path. First attempt used `getTeacherSession()` alone (unreliable in preview) — items stayed locked; then `isTeacherPreview()` alone (`wl_teacher_preview` flag) — worked but a client-writable flag is spoofable. **Final (`d328ae0`, after a HIGH security-review flag): require BOTH** the preview flag (UX) AND `getTeacherSession()` (Supabase-signed JWT — the security gate; students can't forge it). **Verified** via Playwright (Test 2): teacher (both signals) → 7 badge heads "Equip" + legendary equip 0 quarks; spoofer (flag only, no JWT) → `isTeacher=false`, Crown stays "🔒 Badge needed", equip is a no-op. Cosmetic/self-only impact either way (equip is a client write to the student's own RLS-scoped record; `purchase()` stays server-atomic).
