...
- [x] **Teacher full shop access** (`96a6c9d`): a logged-in teacher previewing the scientist shop gets `WL.isTeacher` (via `getTeacherSession()`) → every item across all categories, incl. badge-gated cosmetics, renders owned+unlocked and equips free (no cost/badge check); Surprise-me draws the full catalogue. Student experience unchanged; no badge data faked (student records not polluted). Custom teacher-made items keep their own path. **Pending: live teacher-login View-as-Student spot-check** (a teacher auth session can't be faked in the synthetic harness).
