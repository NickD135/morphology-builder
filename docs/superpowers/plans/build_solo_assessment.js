...
const AVAIL_INFO=13140, AVAIL_PLAIN=14220;   // total box-row height to share among the rows on a page
...
// Properties table (multi-row): colHeaders[0] is the row-label column; rows=[{label,vals:[...]}].
function propsTable(colHeaders,rows,showA){const labelW=2150,valW=Math.floor((5300-labelW)/(colHeaders.length-1));
  const hc=(t,first)=>new TableCell({width:{size:first?labelW:valW,type:WidthType.DXA},borders:cellB,shading:{fill:GRIDHEAD,type:ShadingType.CLEAR},
    margins:{top:46,bottom:46,left:first?80:18,right:18},verticalAlign:VerticalAlign.CENTER,
    children:[para([run(t,{bold:true,size:15})],{after:0,align:first?AlignmentType.LEFT:AlignmentType.CENTER})]});
  const head=new TableRow({children:colHeaders.map((h,i)=>hc(h,i===0))});
  const drows=rows.map(r=>new TableRow({children:[
    new TableCell({width:{size:labelW,type:WidthType.DXA},borders:cellB,margins:{top:46,bottom:46,left:80,right:18},verticalAlign:VerticalAlign.CENTER,children:[para([run(r.label,{size:15})],{after:0})]}),
    ...r.vals.map(v=>new TableCell({width:{size:valW,type:WidthType.DXA},borders:cellB,margins:{top:46,bottom:46,left:18,right:18},verticalAlign:VerticalAlign.CENTER,children:[para([run(showA?String(v):"",{bold:true,color:ANS,size:15})],{after:0,align:AlignmentType.CENTER})]}))
  ]}));
  return new Table({width:{size:labelW+valW*(colHeaders.length-1),type:WidthType.DXA},columnWidths:[labelW,...colHeaders.slice(1).map(()=>valW)],borders:allHair,rows:[head,...drows]});}
// Sketch box: blank box to draw in; on the key shows an italic green note of what to draw.
function sketchBox(label,note,showA,w,h){w=w||4500;h=h||1200;const p=para([run(label,{bold:true})],{after:24});
  const box=new Table({width:{size:w,type:WidthType.DXA},columnWidths:[w],borders:allHair,
    rows:[new TableRow({height:{value:h,rule:"atLeast"},children:[new TableCell({width:{size:w,type:WidthType.DXA},borders:cellB,
      margins:{top:34,bottom:34,left:100,right:100},verticalAlign:VerticalAlign.TOP,
      children:showA?[para([run(note,{italics:true,color:ANS,size:15})],{after:0})]:[para([run(" ")],{after:0})]})]})]});
  return [p,box];}
// -------------------------------------------------------------- layout helpers
function boxCell(b,showA,width,span){width=width||COLW;const s=BAND[b.band];
  ...
  const opt={width:{size:width,type:WidthType.DXA},borders:cellB,margins:{top:56,bottom:90,left:120,right:120},
    verticalAlign:VerticalAlign.TOP,children:[head,...b.build(showA)]};
  if(span)opt.columnSpan=span;
...
function bandRow(band,cols,codes){const s=BAND[band];
  const opt={width:{size:CONTENT,type:WidthType.DXA},borders:cellB,shading:{fill:s.bar,type:ShadingType.CLEAR},
    margins:{top:54,bottom:54,left:170,right:170},verticalAlign:VerticalAlign.CENTER,
    children:[para([run(s.label,{bold:true,size:22,color:WHITE}),run("        "+(codes||s.codes),{bold:true,size:17,color:WHITE})],{after:0})]};
  if(cols===2)opt.columnSpan=2;
  return new TableRow({children:[new TableCell(opt)]});}
function bandTable(band,showA,avail){
  const items=BOXES.filter(b=>b.band===band).sort((a,b)=>ORDER.indexOf(a.code)-ORDER.indexOf(b.code));
  const n=items.length, cols=(n<=3)?1:2, numRows=Math.ceil(n/cols), rowH=Math.floor(avail/Math.max(numRows,1));
  const codes=n?items[0].code+"\u2013"+items[n-1].code:"";
  if(cols===1){
    const rows=[bandRow(band,1,codes)];
    items.forEach(it=>rows.push(new TableRow({height:{value:rowH,rule:"atLeast"},children:[boxCell(it,showA,CONTENT)]})));
    return new Table({width:{size:CONTENT,type:WidthType.DXA},columnWidths:[CONTENT],borders:allHair,rows});
  }
  const rows=[bandRow(band,2,codes)];
  for(let i=0;i<n;i+=2){
    if(i+1<n) rows.push(new TableRow({height:{value:rowH,rule:"atLeast"},children:[boxCell(items[i],showA,COLW),boxCell(items[i+1],showA,COLW)]}));
    else rows.push(new TableRow({height:{value:rowH,rule:"atLeast"},children:[boxCell(items[i],showA,CONTENT,2)]}));
  }
...
function buildChildren(){
  return [
    titleBar(),gap(36),infoStrip(),gap(54),bandTable("RED",false,AVAIL_INFO),pb(),
    titleBar(),gap(54),bandTable("YELLOW",false,AVAIL_PLAIN),pb(),
    titleBar(),gap(54),bandTable("GREEN",false,AVAIL_PLAIN),pb(),
    titleBar(),gap(36),answerBar(),gap(54),bandTable("RED",true,AVAIL_INFO),pb(),
    titleBar(),gap(54),bandTable("YELLOW",true,AVAIL_PLAIN),pb(),
    titleBar(),gap(54),bandTable("GREEN",true,AVAIL_PLAIN),
  ];
}
