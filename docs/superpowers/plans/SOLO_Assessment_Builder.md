...
Bands can hold **any number** of outcomes — Unit 27 is 6 / 6 / 6, Unit 29 is
4 / 6 / 3. The engine adapts automatically: each band's boxes go in two columns
(or one full-width column when a band has 3 or fewer), the rows stretch to fill the
page, an odd last box spans both columns, and the band-bar label ranges to the
actual codes (e.g. `R1–R4`). You don't set any of that — just list the boxes.
...
| Properties of solids (faces/edges/vertices), small data sets | Multi-row table to complete | `propsTable(colHeaders, rows, a)` |
| Sketch views, nets or a diagram | Blank box to draw in (key shows a note of what to draw) | `sketchBox(label, note, a, w, h)` |
...
- **Structure** one band per page; the band's boxes sit in a two-column grid (or
  one full-width column when a band has 3 or fewer) under a band bar; rows stretch
  to fill the page. Test is 3 pages (Red/Yellow/Green), answer key repeats for 3
  more = 6.
...
2. **Row height** — lower `AVAIL_PLAIN` (default `14220`, the total row height
   shared across a band's rows) and/or `AVAIL_INFO` (default `13140`, used on the
   pages that carry the Name/Class strip or the ANSWER KEY strip).
